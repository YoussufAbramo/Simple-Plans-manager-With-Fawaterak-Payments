            ...
            <tr valign="top">
                ...
                <td>
                    ...
                    <div id="fawaterak_test_cards" style="display: <?php echo ($fawaterak_env === 'sandbox') ? 'block' : 'none'; ?>; margin-top: 20px;">
                        ...
                        .card__number__value { color: #fff; font-size: 1.3rem; letter-spacing: 0.08em; white-space: nowrap; }
                        
                        /* Tooltip Copy Button Styling */
                        .qaff-copy-btn { background: rgba(255,255,255,0.1); border: none; border-radius: 4px; color: #fff; cursor: pointer; padding: 6px 8px; display: flex; align-items: center; justify-content: center; position: relative; transition: background 0.2s; }
                        .qaff-copy-btn:hover { background: rgba(255,255,255,0.25); }
                        .qaff-copy-btn svg { width: 16px; height: 16px; fill: currentColor; }
                        .qaff-copy-btn .qaff-tooltip { visibility: hidden; width: 65px; background-color: #333; color: #fff; text-align: center; border-radius: 4px; padding: 4px 0; position: absolute; z-index: 1; bottom: 125%; left: 50%; transform: translateX(-50%); opacity: 0; transition: opacity 0.2s; font-size: 11px; font-family: sans-serif; }
                        .qaff-copy-btn .qaff-tooltip::after { content: ""; position: absolute; top: 100%; left: 50%; margin-left: -4px; border-width: 4px; border-style: solid; border-color: #333 transparent transparent transparent; }
                        ...
                        <!-- Helper function to render a card block cleanly -->
                        <?php 
                        $render_card = function($brand, $num, $exp, $ccv, $logo_svg, $is_fail = false) {
                            $bg_style = $is_fail ? 'style="background-color: #2a1719;"' : '';
                            echo '<div class="card" ' . $bg_style . '>';
                            ...
                            echo '<button type="button" class="qaff-copy-btn" onclick="qaffCopyCard(\'' . esc_js(str_replace(' ', '', $num)) . '\', this)" title="Copy Card Number">';
                        ...
                        <p style="margin: 25px 0 15px 0; font-weight: 600; color: #c53030;">Cards that always return a FAILED transaction:</p>
